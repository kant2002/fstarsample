...
let message = match decodedData with
    | FStar_Pervasives_Native.Some(d) -> (System.String.Concat(Array.ofList(d)) + Environment.NewLine)
    | _ -> failwith "Failed to decode data"
print_string ("After roundtip, value of 'THE ESSENTIAL FEATURE' is:" + Environment.NewLine);;
